// components/heatmap/HeatmapContainer.js
import './Heatmap.css';
import { useEffect, useRef, useCallback } from 'react';
import HeatmapD3 from './Heatmap-d3';

function HeatmapContainer({ heatmapData, dimensions, selectedItems, heatmapControllerMethods }) {
    ...
    const controllerMethods = useCallback({
        updateSelectedItems: (items) => {
            console.log("Heatmap selection updated:", items.length, "items selected");
    ...
    // Component did mount
    useEffect(() => {
        console.log("HeatmapContainer useEffect [] called once the component did mount");
    ...
    useEffect(() => {
        console.log("HeatmapContainer useEffect with dependency [heatmapData, dimensions]");
    ...
    return (
        <div ref={divContainerRef} className="heatmapDivContainer col2">
        </div>
    );
}
